...
class ValueHead(nn.Module):
    def __init__(self, input_dim):
        ...
        # PopArt参数（无需梯度）
        self.mean = nn.Parameter(th.zeros(1), requires_grad=False)
        self.var = nn.Parameter(th.ones(1), requires_grad=False)
    def update_popart(self, targets):  # targets是真实回报
        batch_mean = targets.mean()
        batch_var = targets.var()
        # 更新PopArt统计量
        delta = batch_mean - self.mean
        self.mean.data = self.mean + delta * 0.1  # 滑动更新（系数0.1）
        self.var.data = 0.9 * self.var + 0.1 * batch_var
    def forward(self, x):
        x = self.linear(x)
        # 反归一化：适配真实回报尺度
        return x * th.sqrt(self.var + 1e-8) + self.mean
...
class ActorCriticPolicy(BasePolicy):
    ...
    def _build(self, lr_schedule: Schedule) -> None:
        ...
        if self.popart:
            self.value_net = ValueHead(self.mlp_extractor.latent_dim_vf)
        else:
